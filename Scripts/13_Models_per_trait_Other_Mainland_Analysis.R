...
# It seems that there is no effect of island if we use
# only the other island systems and the effect of herbarium
# is now significant.
...
# The results are non-significant at all.
...
# Depth is in fact significant for other islands and mainland
# Herbarium is not significant for depth
...
# Raw data looks the best
meri_tip_distance_m4 <- lmer(tip_distance ~ other_mainland +
                                     year_collected +
                                     (1|ID),
                             na.action = na.exclude,
                             data=meri_tip_distance_wozero,REML=F)

meri_tip_distance_m5 <- lmer(log(tip_distance + 1) ~ other_mainland +
                                     year_collected +
                                     (1|ID),
                             data=meri_tip_distance_wozero,REML=F)

meri_tip_distance_m6 <- lmer(sqrt(tip_distance) ~ other_mainland +
                                     year_collected +
                                     (1|ID),
                             data=meri_tip_distance_wozero,REML=F)
...
###### Raw data ####
meri_tip_distance_m7 <- lmer(tip_distance ~ other_mainland +
                                     year_collected +
                               Herbarium +
                                     (1|ID),
                             na.action = na.exclude,
...
# Barely significant 0.0551
# non significant for year or herbarium
# This effect wouldn't make sense given that most of sample are herbarium samples
...
# Zero filter data
EM_tip_dist <- emmeans(meri_tip_distance_m7, ~ other_mainland)
...
meri_lower_spines_m1_glmm <- glmmTMB(factor(lower_spines) ~ other_mainland +
                                       Herbarium +
                                             (1|ID),
                                     data = meri_lower_spines,
                                     family = binomial)
...
# No significant results for lower spines
...
#Glmm
EM_lower <- emmeans(meri_lower_spines_m1_glmm, ~ other_mainland, type = "response")
