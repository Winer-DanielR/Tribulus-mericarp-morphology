...
mericarp_scaled <- dplyr::select(mericarp_scaled, c(ID:mericarp_num, 32:36))
mericarp_summary <- mericarp_scaled %>% group_by(ID,mainland_island,island_group) %>% summarize(Length = mean(length),
                                                 Width = mean(width),
                                                 Depth = mean(depth),
                                                 Spine_tip_distance = mean(tip_distance),
                                                 Spine_length = mean(spine_length))
...
mericarp_traits_summary <- select(mericarp_traits_summary, !c(mainland_island))
...
fviz_pca_ind(mericarp_pca, repel = T, geom = c("point"), habillage = mericarp_summary$island_group, palette = "Dark",
...
fviz_pca_biplot(mericarp_pca, repel = T,
                geom = c("point"),
                habillage = mericarp_summary$island_group,
                col.var = "black",
                addEllipses = T
                )
...
biplot1 <- fviz_pca_ind(mericarp_pca,
                        ...
                        pointsize = 3,
                        stroke = 1.5,
                        fill.ind = mericarp_summary$island_group,
                        col.ind = "black",
                        # Color variable by groups
                        legend.title = "Island groups",
                        repel = T,
                        col.var = "black",
                        labelsize = 5,
                        addEllipses = T,
...
biplot2 <- fviz_pca_biplot(mericarp_size_pca,
                        ...
                        pointsize = 4,
                        stroke = 0.5,
                        fill.ind = mericarp_NA_wozero$island_group,
                        col.ind = "black",
                        # Color variable by groups
...
meri_PC1_m1<- lmer(PC1 ~ mainland_island +
                     ...
                     Herbarium +
                      ...
                      REML = F)
meri_PC1_m2<- lmer(PC1 ~ mainland_island +
                     year_collected +
                     Herbarium +
                     Temp +
                     Temp_S +
...
EM_PC_climate <- emmeans(meri_PC1_m2, ~ mainland_island)

### Emmean plot: Length ####
plot(EM_PC1, comparisons = TRUE) + labs(title = "Mericarp Size")
pwpp(EM_PC1)

plot(EM_PC_climate, comparisons = TRUE) + labs(title = "Mericarp Size")
pwpp(EM_PC_climate)
...
figure_mericarp_PC1_lower <- ggarrange(ggplot_PC2,
                                       ggplot_lower,
                                       labels = c("A", "B"),
                                       ncol = 2,
                                       nrow = 1) + 
